...
export default function SortableRangeItem({ 
    ...
    const fromError = parseInt(range.from, 10) > totalPages || parseInt(range.from, 10) < 1;
    const toError = parseInt(range.to, 10) > totalPages || parseInt(range.to, 10) < 1;
    ...
    return (
        <div 
            ...
            className={`flex items-center gap-1 p-1.5 rounded border transition-all cursor-pointer ${
                ...
                isActive ? `${colorClass} ring-2 ring-offset-1 ring-blue-400` : 
        ...
        >
            ...
            <span className="text-[10px] font-semibold text-slate-500 w-10 shrink-0">R{index + 1}</span>
