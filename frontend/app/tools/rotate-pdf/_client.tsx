...
export default function RotatePdfPage() {
    ...
    const handleRotatePages = async () => {
        ...
        try {
            const response = await fetch('/api/tools/rotate-pdf/rotate-pages', {
                method: 'POST',
                body: formData,
            });
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                throw new Error(errorData.details || errorData.error || 'Rotate failed');
            }
            const blob = await response.blob();
            downloadBlob(blob, 'rotated.pdf');
        } catch (error) {
            console.error(error);
            alert(error instanceof Error ? error.message : 'Failed to rotate PDF.');
        } finally {
    ...
    const handleRotateAll = async () => {
        ...
        try {
            const response = await fetch('/api/tools/rotate-pdf/rotate-all', {
                method: 'POST',
                body: formData,
            });
            if (!response.ok) {
