...
import { Upload, FileText, ArrowRight, Download, RefreshCw, Trash2, DownloadCloud } from "lucide-react";
...
export default function PptToPdfPage() {
    ...
    const convertItem = async (item: FileQueueItem) => {
        ...
        try {
            const response = await fetch('/api/tools/ppt-to-pdf/convert', {
                method: 'POST',
                body: formData,
            });
    ...
    const handleDownloadAll = async () => {
        ...
        link.href = url;
        link.download = `converted_pdfs_${new Date().getTime()}.zip`;
    ...
    return (
        <>
            <ToolPageHeader title="PPT to PDF" />
            <div className="py-8 md:py-12">
            <div className="container mx-auto px-4 max-w-5xl">
                <div className="text-center mb-10">
                    <div className="inline-flex items-center justify-center p-3 bg-orange-100 text-orange-600 rounded-xl mb-6">
                        <FileText className="w-8 h-8" />
                    </div>
                    <h1 className="text-4xl font-extrabold text-slate-900 mb-4">PPT to PDF</h1>
                    <p className="text-lg text-slate-600 max-w-2xl mx-auto">
                        Convert PowerPoint presentations to PDF format. Preserve your slides perfectly.
                    </p>
                </div>
                ...
                >
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                        <div className={`p-3 bg-orange-50 text-orange-600 rounded-xl ${hasItems ? 'w-10 h-10' : 'w-16 h-16'}`}>
                            <Upload className={hasItems ? 'w-5 h-5' : 'w-8 h-8'} />
                        </div>
                        <div>
                            <h3 className={`font-bold text-slate-900 ${hasItems ? 'text-lg' : 'text-xl'}`}>
                                {hasItems ? 'Add more files' : 'Drop your PowerPoint files here'}
                            </h3>
                            {!hasItems && (
                            ...
                            <div key={item.id} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex flex-col gap-4 transition-all hover:shadow-md">
                                ...
                                <div className="w-full flex justify-end">
                                    {item.status === 'converting' && (
                                        <div className="flex items-center text-orange-600 font-medium px-4">
