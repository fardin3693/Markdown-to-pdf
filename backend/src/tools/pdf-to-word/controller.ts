...
export const convertPdfToWordController = async (req: Request, res: Response) => {
    if (!req.file) {
        return res.status(400).json({ error: 'No file uploaded' });
    }

    const inputPath = req.file.path;
    const outputFilename = `${path.parse(req.file.filename).name}.docx`;
    ...
    try {
        ...
        res.status(500).json({
            error: 'Failed to convert PDF to document',
