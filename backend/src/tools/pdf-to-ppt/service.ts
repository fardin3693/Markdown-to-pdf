...
export const convertPdfToPpt = async (inputPath: string, outputPath: string): Promise<void> => {
    ...
    const cmd = `"${libreOfficePath}" --headless --convert-to pptx --outdir "${dir}" "${inputPath}"`;

    try {
        await execAsync(cmd);
        
        const libreOfficeOutput = path.join(dir, `${inputBasenameWithoutExt}.pptx`);
        ...
        if (await fs.pathExists(libreOfficeOutput)) {
            ...
            throw new Error(`PPTX was not created. Expected at: ${libreOfficeOutput}`);
        }
    } catch (error: any) {
